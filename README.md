...
| **Firebase Storage** | Hero / OGP 画像のファイル保存 |
...
構築 → 「Firestore Database」→ データベースの作成 → Standard エディション → `asia-northeast1` → 本番環境モードで開始する →　作成

**2. Authentication**

構築 → 「Authentication」→ 始める → **メール/パスワード** を有効にする。また、**Google** も必要に応じて有効にし、プロジェクトのサポートメールを設定して保存します。
...
npm run init
```
> **Mac**: 解凍したフォルダを右クリック → サービス →「フォルダに新規ターミナルタブ」を選択
...
> 💡 **日本語入力でターミナルが不安定になる場合**
> ターミナルでの日本語入力（IME変換）時に表示が乱れたりクラッシュしたりする場合は、以下のように引数でパラメータを直接渡すことができます。
> ```bash
> npm run init \
>   --project "your-project-id" \
>   --firebase-user "your@mail.com" \
>   --title "AIハッカソン" \
>   --subtitle "ハッカソン概要" \
>   --admin-user admin \
>   --admin-pass password123 \
>   --admin-group "mail1@example.com,mail2@example.com"
> ```
> 引数で指定した項目は入力がスキップされるため、安全にセットアップを完了できます。
...
- **`npm run init`**: 初期設定を一括で行います。
    - パラメータ指定の例： 
    `npm run init -- --project "ID" --title "名前" --user "admin" --pass "PW"`
